-- 已应用过旧版 V2 的库：将短名称系统分类重命名，并统一为英文 icon key；再补全与新版一致的系统分类

-- 支出：旧名 -> 新名 + icon
UPDATE category SET name = '餐饮美食', icon = 'food_dining', updated_at = NOW()
WHERE user_id = 0 AND type = 1 AND name = '餐饮';

UPDATE category SET name = '购物消费', icon = 'shopping', updated_at = NOW()
WHERE user_id = 0 AND type = 1 AND name = '购物';

UPDATE category SET name = '交通出行', icon = 'transport', updated_at = NOW()
WHERE user_id = 0 AND type = 1 AND name = '交通';

UPDATE category SET name = '居家生活', icon = 'home_life', updated_at = NOW()
WHERE user_id = 0 AND type = 1 AND name = '住房';

UPDATE category SET name = '休闲娱乐', icon = 'entertainment', updated_at = NOW()
WHERE user_id = 0 AND type = 1 AND name = '娱乐';

UPDATE category SET name = '医疗健康', icon = 'medical', updated_at = NOW()
WHERE user_id = 0 AND type = 1 AND name = '医疗';

UPDATE category SET name = '学习教育', icon = 'education', updated_at = NOW()
WHERE user_id = 0 AND type = 1 AND name = '教育';

UPDATE category SET name = '话费网费', icon = 'phone_bill', updated_at = NOW()
WHERE user_id = 0 AND type = 1 AND name = '通讯';

-- 收入：旧名 -> 新名 + icon
UPDATE category SET name = '职业薪水', icon = 'salary', updated_at = NOW()
WHERE user_id = 0 AND type = 2 AND name = '工资';

UPDATE category SET name = '奖金福利', icon = 'bonus', updated_at = NOW()
WHERE user_id = 0 AND type = 2 AND name = '奖金';

UPDATE category SET name = '兼职外快', icon = 'part_time', updated_at = NOW()
WHERE user_id = 0 AND type = 2 AND name = '兼职';

UPDATE category SET name = '其他收入', icon = 'other_income', updated_at = NOW()
WHERE user_id = 0 AND type = 2 AND name = '理财';

-- 与新版 V2 全量一致：只插入尚不存在的系统分类
-- 为避免历史库排序不一致，先统一系统支出分类排序（仅影响 user_id=0）
...
UPDATE category SET sort = 81, updated_at = NOW() WHERE user_id = 0 AND type = 1 AND name = '其他支出';
